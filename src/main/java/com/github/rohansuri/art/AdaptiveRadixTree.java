...
public class AdaptiveRadixTree<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V> {
	...
	private LeafNode<K, V> getEntry(Node node, byte[] key) {
		...
		while (true) {
			if (node instanceof LeafNode) {
				...
				if (arraysEqual(leafBytes, startFrom, leafBytes.length, key, startFrom, key.length)) {
					return leaf;
				}
				return null;
			}
	...
	private boolean arraysEqual(byte[] a, int aFromIndex, int aToIndex, byte[] b, int bFromIndex, int bToIndex) {
	...
	private boolean arraysEqual(byte[] a, int aFromIndex, byte[] b, int bFromIndex, int length) {
		if (a.length - aFromIndex < length || b.length - bFromIndex < length) {
			return false;
		}
